##USAGE##
# python signalEfficiency_plotter.py -S inputDirs/df_output_v18_12-02-01_cl68pdf_privateSIG  -e 2018 -t v18_12-02-01_cl68pdf_privateSIG
...
for cat in ('1b','2b'):
    ...
    for iFile in sig_list:
        ...
        if mA_ == 600:
            ...
            if runOn2016:
                sig_eff_ma_600.update({int(rootFile.split('_')[4].strip('Ma')):hist_eff})
            else:
                sig_eff_ma_600.update({int(rootFile.split('_')[9].strip('.root')):hist_eff})
        if mA_ == 1200:
            #print (mA_)
            hist = fin.Get("h_reg_SR_"+cat+"_MET")
            hist_total = fin.Get("h_total_mcweight")
            hist_eff = hist.Integral()/hist_total.Integral()
            if runOn2016:
                sig_eff_ma_1200.update({int(rootFile.split('_')[4].strip('Ma')):hist_eff})
            else:
                sig_eff_ma_1200.update({int(rootFile.split('_')[9].strip('.root')):hist_eff})
    ...
    c1.Update()
    c1.SaveAs('outPlotDir/Sig_Eff_'+sr+'_'+plot_tag+'.pdf')
    c1.Close()
