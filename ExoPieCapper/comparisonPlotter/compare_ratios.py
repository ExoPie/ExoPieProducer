...
list_comp = []
tobe_comp = ['diboson', 'qcd', 'singlet', 'smh', 'tt', 'wjets', 'zjets']
# tobe_comp = ['Jet1Phi', 'METPhi']
# tobe_comp = ['Recoil']
# tobe_comp = ['NTau']

listfiles_1 = [f for f in os.listdir(options.FirstDir+'/png/')]
listfiles_2 = [f for f in os.listdir(options.SecondDir+'/png/')]
listfiles_3 = [f for f in os.listdir(options.ThirdDir+'/png/')]
list_temp = []
...
for i in listfiles_1:
  if (i in listfiles_1 and i in listfiles_2 and i in listfiles_3) and [hist for hist in tobe_comp if(hist in i)]:
    im_list = [options.FirstDir+'/png/' +i, options.SecondDir+'/png/'+i, options.ThirdDir+'/png/'+i]
...
print(list_comp)
if not os.path.exists('compared_files/bbDMPng/'):
    os.makedirs('compared_files/bbDMPng/')
if not os.path.exists('compared_files/bbDMPdf/'):
    os.makedirs('compared_files/bbDMPdf/')
...
for im in list_comp:
    ...
    imgs_comb.save('compared_files/bbDMPng/'+image_name+'.png', optimize=True, quality=100)
    imgs_comb.save('compared_files/bbDMPdf/'+image_name+'.pdf', optimize=True, quality=100)
