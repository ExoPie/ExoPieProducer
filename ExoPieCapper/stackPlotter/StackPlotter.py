...
def makeplot(loc, hist, titleX, XMIN, XMAX, Rebin, ISLOG, NORATIOPLOT, reg, varBin, row=2):
    ...
    for file in files.readlines()[:]:
        ...
        elif 'ZJetsToNuNu' in file or 'Z1JetsToNuNu' in file or 'Z2JetsToNuNu' in file or 'DYJetsToNuNu_PtZ' in file:
            xsec = sample_xsec.getXsec(file, options.year)
            print ('file', file ,'xsec', xsec,'total_events',total_events,'\n')
            if (total_events > 0):
                normlisation = (xsec*luminosity)/(total_events)
    ...
    if makeMuCHplots:
        ...
        noYieldHisto = bool(('weight' in hist) or ('_up' in hist)
                            or ('_down' in hist))
    if makeSIGplots:
        if ('_'+limit_varSR in hist or 'cutFlow' in hist) and ('SR' in hist) and not noYieldHisto:
        # if ('nJets' in hist) and ('SR' in hist):
            ...
            for i in range(1,Stackhist.GetNbinsX()+1):
                # bin_ = math.sqrt(Stackhist.GetBinContent(i))
                bin_ = Stackhist.GetBinContent(i)
                sqbkg.SetBinContent(i, bin_)
            # [sig_hist[i].Divide(sqbkg) for i in sig_hist]
...
#####================================= data section =========================
    ...
    if 'QCD' in hist: legend.AddEntry(QCD, QCDLegend, "f")
    ...
    bkg_list = { 'Top': Top, 'STop': STop, 'WJets': WJets, 'DIBOSON': DIBOSON, 'GJets': GJets, 'ZJets': ZJets, 'DYJets': DYJets, 'SMH': SMH,'Total_Bkg': Stackhist,'data_obs': h_data}
    ...
    if noPdfPng and not datainSR:
        for key in bkg_list:
            ...
            hist_yield.write(str(bkg_list_name[key])+' '+str.format('{0:.2f}', bkg_list[key].GetBinContent(1))+'\xb1'+str.format('{0:.2f}', bkg_list[key].GetBinError(1))+'\n')
        hist_yield.close()
...
for reg in regions:
    ...
    if 'SR_' in reg or 'preselR' in reg or 'QCDbCR' in reg:
        makeplot('reg_'+reg+'_'+limit_varSR, 'h_reg_'+reg+'_'+limit_varSR,
        ...
        if ('SR_2b' in reg):
                makeplot('reg_'+reg+'_MET', 'h_reg_'+reg+'_MET',
                        'p_{T}^{miss} (GeV)', 250, 1000, rebin, isLog, 0, reg, varBin=False)
        if systHistoFiles:
            makeplot('reg_'+reg+'_'+limit_varSR+'_CMSyear_eff_bUp', 'h_reg_'+reg+'_'+limit_varSR+'_CMSyear_eff_bUp',
                    var_legendSR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_'+limit_varSR+'_CMSyear_eff_bDown', 'h_reg_'+reg+'_'+limit_varSR+'_CMSyear_eff_bDown',
                    var_legendSR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_'+limit_varSR+'_CMSyear_fake_bUp', 'h_reg_'+reg+'_'+limit_varSR+'_CMSyear_fake_bUp',
                    var_legendSR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_'+limit_varSR+'_CMSyear_fake_bDown', 'h_reg_'+reg+'_'+limit_varSR+'_CMSyear_fake_bDown',
                    var_legendSR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_'+limit_varSR+'_EWKUp', 'h_reg_'+reg+'_'+limit_varSR+'_EWKUp',
                    var_legendSR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_'+limit_varSR+'_EWKDown', 'h_reg_'+reg+'_'+limit_varSR+'_EWKDown',
                    var_legendSR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_'+limit_varSR+'_CMSyear_TopUp', 'h_reg_'+reg+'_'+limit_varSR+'_CMSyear_TopUp',
                    var_legendSR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_'+limit_varSR+'_CMSyear_TopDown', 'h_reg_'+reg+'_'+limit_varSR+'_CMSyear_TopDown',
                    var_legendSR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_'+limit_varSR+'_CMSyear_trig_metUp', 'h_reg_'+reg+'_'+limit_varSR+'_CMSyear_trig_metUp',
                    var_legendSR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_'+limit_varSR+'_CMSyear_trig_metDown', 'h_reg_'+reg+'_'+limit_varSR+'_CMSyear_trig_metDown',
                    var_legendSR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_'+limit_varSR+'_CMSyear_PUUp', 'h_reg_'+reg+'_'+limit_varSR+'_CMSyear_PUUp',
                    var_legendSR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_'+limit_varSR+'_CMSyear_PUDown', 'h_reg_'+reg+'_'+limit_varSR+'_CMSyear_PUDown',
                    var_legendSR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_'+limit_varSR+'_JECUp', 'h_reg_'+reg+'_'+limit_varSR+'_JECUp',
                    var_legendSR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_'+limit_varSR+'_JECDown', 'h_reg_'+reg+'_'+limit_varSR+'_JECDown',
                    var_legendSR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_'+limit_varSR+'_JECAbsoluteUp', 'h_reg_'+reg+'_'+limit_varSR+'_JECAbsoluteUp',
                    var_legendSR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_'+limit_varSR+'_JECAbsolute_yearUp', 'h_reg_'+reg +
                    '_'+limit_varSR+'_JECAbsolute_yearUp', var_legendSR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_'+limit_varSR+'_JECBBEC1Up', 'h_reg_'+reg+'_'+limit_varSR+'_JECBBEC1Up',
                    var_legendSR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_'+limit_varSR+'_JECBBEC1_yearUp', 'h_reg_'+reg+'_'+limit_varSR+'_JECBBEC1_yearUp',
                    var_legendSR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_'+limit_varSR+'_JECEC2Up', 'h_reg_'+reg+'_'+limit_varSR+'_JECEC2Up',
                    var_legendSR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_'+limit_varSR+'_JECEC2_yearUp', 'h_reg_'+reg+'_'+limit_varSR+'_JECEC2_yearUp',
                    var_legendSR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_'+limit_varSR+'_JECFlavorQCDUp', 'h_reg_'+reg+'_'+limit_varSR+'_JECFlavorQCDUp',
                    var_legendSR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_'+limit_varSR+'_JECHFUp', 'h_reg_'+reg+'_'+limit_varSR+'_JECHFUp',
                    var_legendSR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_'+limit_varSR+'_JECHF_yearUp', 'h_reg_'+reg+'_'+limit_varSR+'_JECHF_yearUp',
                    var_legendSR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_'+limit_varSR+'_JECRelativeBalUp', 'h_reg_'+reg+'_'+limit_varSR+'_JECRelativeBalUp',
                    var_legendSR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_'+limit_varSR+'_JECRelativeSample_yearUp', 'h_reg_'+reg +
                    '_'+limit_varSR+'_JECRelativeSample_yearUp', var_legendSR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_'+limit_varSR+'_JECAbsoluteDown', 'h_reg_'+reg+'_'+limit_varSR+'_JECAbsoluteDown',
                    var_legendSR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_'+limit_varSR+'_JECAbsolute_yearDown', 'h_reg_'+reg +
                    '_'+limit_varSR+'_JECAbsolute_yearDown', var_legendSR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_'+limit_varSR+'_JECBBEC1Down', 'h_reg_'+reg+'_'+limit_varSR+'_JECBBEC1Down',
                    var_legendSR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_'+limit_varSR+'_JECBBEC1_yearDown', 'h_reg_'+reg +
                    '_'+limit_varSR+'_JECBBEC1_yearDown', var_legendSR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_'+limit_varSR+'_JECEC2Down', 'h_reg_'+reg+'_'+limit_varSR+'_JECEC2Down',
                    var_legendSR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_'+limit_varSR+'_JECEC2_yearDown', 'h_reg_'+reg+'_'+limit_varSR+'_JECEC2_yearDown',
                    var_legendSR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_'+limit_varSR+'_JECFlavorQCDDown', 'h_reg_'+reg+'_'+limit_varSR+'_JECFlavorQCDDown',
                    var_legendSR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_'+limit_varSR+'_JECHFDown', 'h_reg_'+reg+'_'+limit_varSR+'_JECHFDown',
                    var_legendSR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_'+limit_varSR+'_JECHF_yearDown', 'h_reg_'+reg+'_'+limit_varSR+'_JECHF_yearDown',
                    var_legendSR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_'+limit_varSR+'_JECRelativeBalDown', 'h_reg_'+reg +
                    '_'+limit_varSR+'_JECRelativeBalDown', var_legendSR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_'+limit_varSR+'_JECRelativeSample_yearDown', 'h_reg_'+reg +
                    '_'+limit_varSR+'_JECRelativeSample_yearDown', var_legendSR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_'+limit_varSR+'_ResUp', 'h_reg_'+reg+'_'+limit_varSR+'_ResUp',
                    var_legendSR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_'+limit_varSR+'_ResDown', 'h_reg_'+reg+'_'+limit_varSR+'_ResDown',
                    var_legendSR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_'+limit_varSR+'_EnUp', 'h_reg_'+reg+'_'+limit_varSR+'_EnUp',
                    var_legendSR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_'+limit_varSR+'_EnDown', 'h_reg_'+reg+'_'+limit_varSR+'_EnDown',
                    var_legendSR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            ...
            makeplot('reg_'+reg+'_'+limit_varSR+'_CMSyear_mu_scaleDown', 'h_reg_'+reg+'_'+limit_varSR+'_CMSyear_mu_scaleDown',
                    var_legendSR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_'+limit_varSR+'_CMSyear_pdfUp', 'h_reg_'+reg+'_'+limit_varSR+'_CMSyear_pdfUp',
                    var_legendSR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_'+limit_varSR+'_CMSyear_pdfDown', 'h_reg_'+reg+'_'+limit_varSR+'_CMSyear_pdfDown',
                    var_legendSR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_'+limit_varSR+'_CMSyear_prefireUp', 'h_reg_'+reg+'_'+limit_varSR+'_CMSyear_prefireUp',
                    var_legendSR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_'+limit_varSR+'_CMSyear_prefireDown', 'h_reg_'+reg+'_'+limit_varSR+'_CMSyear_prefireDown',
                    var_legendSR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
        if allplots:
            ...
            makeplot('reg_'+reg+'_nPV', 'h_reg_'+reg+'_nPV',
                    'Before PU reweighting', 0., 70., 2, 1, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_PUnPV', 'h_reg_'+reg+'_PUnPV',
                    'After PU reweighting', 0., 70., 2, 1, 0, reg, varBin=False)
            if ('SR_1b' in reg):
                ...
                makeplot('reg_'+reg+'_isjet2EtaMatch', 'h_reg_'+reg+'_isjet2EtaMatch',
                        'JET1#eta X JET3#eta', -1, 1, rebin, isLog, 0, reg, varBin=False)
        ...
        makeplot('reg_'+reg+'_'+limit_varCR, 'h_reg_'+reg+'_'+limit_varCR,
                var_legendCR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
        makeplot('reg_'+reg+'_lep1_pT', 'h_reg_'+reg+'_lep1_pT',
                'lepton1 p_{T}', 30, 500, rebin, 1, 0, reg, varBin=False)
        # makeplot('reg_'+reg+'_prod_cat', 'h_reg_'+reg+'_prod_cat', 'Jet Flavour', 0, 3, 1, 1, 0, reg, varBin=False)
        if systHistoFiles:
            makeplot('reg_'+reg+'_'+limit_varCR+'_CMSyear_eff_bUp', 'h_reg_'+reg+'_'+limit_varCR+'_CMSyear_eff_bUp',
                    var_legendCR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_'+limit_varCR+'_CMSyear_eff_bDown', 'h_reg_'+reg+'_'+limit_varCR+'_CMSyear_eff_bDown',
                    var_legendCR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_'+limit_varCR+'_CMSyear_fake_bUp', 'h_reg_'+reg+'_'+limit_varCR+'_CMSyear_fake_bUp',
                    var_legendCR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_'+limit_varCR+'_CMSyear_fake_bDown', 'h_reg_'+reg+'_'+limit_varCR+'_CMSyear_fake_bDown',
                    var_legendCR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_'+limit_varCR+'_EWKUp', 'h_reg_'+reg+'_'+limit_varCR+'_EWKUp',
                    var_legendCR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_'+limit_varCR+'_EWKDown', 'h_reg_'+reg+'_'+limit_varCR+'_EWKDown',
                    var_legendCR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_'+limit_varCR+'_CMSyear_TopUp', 'h_reg_'+reg+'_'+limit_varCR+'_CMSyear_TopUp',
                    var_legendCR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_'+limit_varCR+'_CMSyear_TopDown', 'h_reg_'+reg+'_'+limit_varCR+'_CMSyear_TopDown',
                    var_legendCR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_'+limit_varCR+'_CMSyear_trig_metUp', 'h_reg_'+reg +
                    '_'+limit_varCR+'_CMSyear_trig_metUp', var_legendCR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_'+limit_varCR+'_CMSyear_trig_metDown', 'h_reg_'+reg +
                    '_'+limit_varCR+'_CMSyear_trig_metDown', var_legendCR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_'+limit_varCR+'_CMSyear_PUUp', 'h_reg_'+reg+'_'+limit_varCR+'_CMSyear_PUUp',
                    var_legendCR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_'+limit_varCR+'_CMSyear_PUDown', 'h_reg_'+reg+'_'+limit_varCR+'_CMSyear_PUDown',
                    var_legendCR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            # makeplot('reg_'+reg+'_'+limit_varCR+'_JECUp', 'h_reg_'+reg+'_'+limit_varCR+'_JECUp',
            #          var_legendCR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            # makeplot('reg_'+reg+'_'+limit_varCR+'_JECDown', 'h_reg_'+reg+'_'+limit_varCR+'_JECDown',
            #          var_legendCR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_'+limit_varCR+'_JECAbsoluteUp', 'h_reg_'+reg +
                    '_'+limit_varCR+'_JECAbsoluteUp', var_legendCR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_'+limit_varCR+'_JECAbsolute_yearUp', 'h_reg_'+reg +
                    '_'+limit_varCR+'_JECAbsolute_yearUp', var_legendCR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_'+limit_varCR+'_JECBBEC1Up', 'h_reg_'+reg +
                    '_'+limit_varCR+'_JECBBEC1Up', var_legendCR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_'+limit_varCR+'_JECBBEC1_yearUp', 'h_reg_'+reg +
                    '_'+limit_varCR+'_JECBBEC1_yearUp', var_legendCR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_'+limit_varCR+'_JECEC2Up', 'h_reg_'+reg +
                    '_'+limit_varCR+'_JECEC2Up', var_legendCR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_'+limit_varCR+'_JECEC2_yearUp', 'h_reg_'+reg +
                    '_'+limit_varCR+'_JECEC2_yearUp', var_legendCR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_'+limit_varCR+'_JECFlavorQCDUp', 'h_reg_'+reg +
                    '_'+limit_varCR+'_JECFlavorQCDUp', var_legendCR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_'+limit_varCR+'_JECHFUp', 'h_reg_'+reg+'_'+limit_varCR+'_JECHFUp',
                    var_legendCR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_'+limit_varCR+'_JECHF_yearUp', 'h_reg_'+reg +
                    '_'+limit_varCR+'_JECHF_yearUp', var_legendCR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_'+limit_varCR+'_JECRelativeBalUp', 'h_reg_'+reg +
                    '_'+limit_varCR+'_JECRelativeBalUp', var_legendCR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_'+limit_varCR+'_JECRelativeSample_yearUp', 'h_reg_'+reg +
                    '_'+limit_varCR+'_JECRelativeSample_yearUp', var_legendCR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_'+limit_varCR+'_JECAbsoluteDown', 'h_reg_'+reg +
                    '_'+limit_varCR+'_JECAbsoluteDown', var_legendCR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_'+limit_varCR+'_JECAbsolute_yearDown', 'h_reg_'+reg +
                    '_'+limit_varCR+'_JECAbsolute_yearDown', var_legendCR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_'+limit_varCR+'_JECBBEC1Down', 'h_reg_'+reg +
                    '_'+limit_varCR+'_JECBBEC1Down', var_legendCR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_'+limit_varCR+'_JECBBEC1_yearDown', 'h_reg_'+reg +
                    '_'+limit_varCR+'_JECBBEC1_yearDown', var_legendCR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_'+limit_varCR+'_JECEC2Down', 'h_reg_'+reg +
                    '_'+limit_varCR+'_JECEC2Down', var_legendCR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_'+limit_varCR+'_JECEC2_yearDown', 'h_reg_'+reg +
                    '_'+limit_varCR+'_JECEC2_yearDown', var_legendCR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_'+limit_varCR+'_JECFlavorQCDDown', 'h_reg_'+reg +
                    '_'+limit_varCR+'_JECFlavorQCDDown', var_legendCR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_'+limit_varCR+'_JECHFDown', 'h_reg_'+reg +
                    '_'+limit_varCR+'_JECHFDown', var_legendCR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_'+limit_varCR+'_JECHF_yearDown', 'h_reg_'+reg +
                    '_'+limit_varCR+'_JECHF_yearDown', var_legendCR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_'+limit_varCR+'_JECRelativeBalDown', 'h_reg_'+reg +
                    '_'+limit_varCR+'_JECRelativeBalDown', var_legendCR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_'+limit_varCR+'_JECRelativeSample_yearDown', 'h_reg_'+reg +
                    '_'+limit_varCR+'_JECRelativeSample_yearDown', var_legendCR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            # makeplot('reg_'+reg+'_'+limit_varCR+'_ResUp', 'h_reg_'+reg+'_'+limit_varCR+'_ResUp',
            #          var_legendCR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            # makeplot('reg_'+reg+'_'+limit_varCR+'_ResDown', 'h_reg_'+reg+'_'+limit_varCR+'_ResDown',
            #          var_legendCR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_'+limit_varCR+'_EnUp', 'h_reg_'+reg+'_'+limit_varCR+'_EnUp',
                    var_legendCR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_'+limit_varCR+'_EnDown', 'h_reg_'+reg+'_'+limit_varCR+'_EnDown',
                    var_legendCR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_'+limit_varCR+'_CMSyear_mu_scaleUp', 'h_reg_'+reg+'_'+limit_varCR+'_CMSyear_mu_scaleUp',
                    var_legendCR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_'+limit_varCR+'_CMSyear_mu_scaleDown', 'h_reg_'+reg+'_'+limit_varCR+'_CMSyear_mu_scaleDown',
                    var_legendCR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_'+limit_varCR+'_CMSyear_pdfUp', 'h_reg_'+reg+'_'+limit_varCR+'_CMSyear_pdfUp',
                    var_legendCR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_'+limit_varCR+'_CMSyear_pdfDown', 'h_reg_'+reg+'_'+limit_varCR+'_CMSyear_pdfDown',
                    var_legendCR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_'+limit_varCR+'_CMSyear_prefireUp', 'h_reg_'+reg+'_'+limit_varCR+'_CMSyear_prefireUp',
                    var_legendCR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_'+limit_varCR+'_CMSyear_prefireDown', 'h_reg_'+reg+'_'+limit_varCR+'_CMSyear_prefireDown',
                    var_legendCR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_'+limit_varCR+'_CMSyear_trig_eleUp', 'h_reg_'+reg+'_'+limit_varCR+'_CMSyear_trig_eleUp',
                    var_legendCR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_'+limit_varCR+'_CMSyear_trig_eleDown', 'h_reg_'+reg+'_'+limit_varCR+'_CMSyear_trig_eleDown',
                    var_legendCR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_'+limit_varCR+'_CMSyear_EleIDUp', 'h_reg_'+reg+'_'+limit_varCR+'_CMSyear_EleIDUp',
                    var_legendCR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_'+limit_varCR+'_CMSyear_EleIDDown', 'h_reg_'+reg+'_'+limit_varCR+'_CMSyear_EleIDDown',
                    var_legendCR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_'+limit_varCR+'_CMSyear_EleRECOUp', 'h_reg_'+reg+'_'+limit_varCR+'_CMSyear_EleRECOUp',
                    var_legendCR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_'+limit_varCR+'_CMSyear_EleRECODown', 'h_reg_'+reg+'_'+limit_varCR+'_CMSyear_EleRECODown',
                    var_legendCR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_'+limit_varCR+'_CMSyear_MuIDUp', 'h_reg_'+reg+'_'+limit_varCR+'_CMSyear_MuIDUp',
                    var_legendCR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_'+limit_varCR+'_CMSyear_MuIDDown', 'h_reg_'+reg+'_'+limit_varCR+'_CMSyear_MuIDDown',
                    var_legendCR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_'+limit_varCR+'_CMSyear_MuISOUp', 'h_reg_'+reg+'_'+limit_varCR+'_CMSyear_MuISOUp',
                        var_legendCR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_'+limit_varCR+'_CMSyear_MuISODown', 'h_reg_'+reg+'_'+limit_varCR+'_CMSyear_MuISODown',
                        var_legendCR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_'+limit_varCR+'_CMSyear_MuTRKUp', 'h_reg_'+reg+'_'+limit_varCR+'_CMSyear_MuTRKUp',
                    var_legendCR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_'+limit_varCR+'_CMSyear_MuTRKDown', 'h_reg_'+reg+'_'+limit_varCR+'_CMSyear_MuTRKDown',
                    var_legendCR, minBin, maxBin, rebin, isLog, 0, reg, varBin=False)
        if allplots:
            makeplot('reg_'+reg+'_cutFlow', 'h_reg_'+reg+'_cutFlow',
                'CutFlow', 0, 12, rebin, isLog, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_MET', 'h_reg_'+reg+'_MET',
                    'Real p_{T}^{miss} (GeV)', 0., 700., rebin, 1, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_METPhi', 'h_reg_'+reg+'_METPhi',
                    ' p_{T}^{miss} #phi', -3, 3, rebin, isLog, 0, reg, varBin=False)
            # makeplot('reg_'+reg+'_dPhiTrk_pfMET', 'h_reg_'+reg+'_dPhiTrk_pfMET',
            #          '#Delta#phi (Trkp_{T}^{miss} - p_{T}^{miss})', -3.2, 3.2, rebin, 1, 0, reg, varBin=False)
            # makeplot('reg_'+reg+'_dPhiCalo_pfMET', 'h_reg_'+reg+'_dPhiCalo_pfMET',
            #          '#Delta#phi(Calop_{T}^{miss} - pfp_{T}^{miss})', -3.2, 3.2, rebin, 1, 0, reg, varBin=False)
            ...
            makeplot('reg_'+reg+'_lep1_pT', 'h_reg_'+reg+'_lep1_pT',
                    'lepton1 p_{T}', 30, 500, rebin, 1, 0, reg, varBin=False)
            # makeplot('reg_'+reg+'_lep1_pT_CMSyear_EleIDUp', 'h_reg_'+reg+'_lep1_pT_CMSyear_EleIDUp',
            #         'lepton1 p_{T}', 0, 500, rebin, 1, 0, reg, varBin=False)
            # makeplot('reg_'+reg+'_lep1_pT_CMSyear_EleIDDown', 'h_reg_'+reg+'_lep1_pT_CMSyear_EleIDDown',
            #         'lepton1 p_{T}', 0, 500, rebin, 1, 0, reg, varBin=False)
            ...
            if ('Z' in reg):
                ...
                makeplot('reg_'+reg+'_lep2_pT', 'h_reg_'+reg+'_lep2_pT',
                        'lepton2 p_{T}', 30, 500, 1, 1, 0, reg, varBin=False)
            ...
            if ('1b' in reg or '2j' in reg) and ('WmunuCR_1b' not in reg) and ('WenuCR_1b' not in reg):
                ...
                makeplot('reg_'+reg+'_Recoil', 'h_reg_'+reg+'_Recoil',
                        'Recoil (GeV)', 250, 1000, rebin, isLog, 0, reg, varBin=False)
                makeplot('reg_'+reg+'_isjet2EtaMatch', 'h_reg_'+reg+'_isjet2EtaMatch',
                        'JET1#eta X JET3#eta', -1, 1, rebin, isLog, 0, reg, varBin=False)
                makeplot('reg_'+reg+'_M_Jet1Jet3', 'h_reg_'+reg+'_M_Jet1Jet3',
                        'Inv Mass(Jet1, Jet3)', 0, 2000, 5, 1, 0, reg, varBin=False)
            makeplot('reg_'+reg+'_rJet1PtMET', 'h_reg_'+reg+'_rJet1PtMET',
                    'Jet1 p_{T}/MET', 0, 10, rebin, isLog, 0, reg, varBin=False)
    # except:
    #     pass
yield_outfile.close()
